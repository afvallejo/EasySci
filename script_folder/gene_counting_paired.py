'''
This script takes the files from the duplicate removed folder, extracts the single cells and then counts the gene expressions per cell for the paired-end pipeline.
'''
...
def EasySci_count(input_folder, output_folder, exons, genes, gene_end, gene_annotat, randomN_barcodes, sample):
    ...
    for bundle in HTSeq.pair_SAM_alignments( almnt_file, bundle=True ):
        ...
        if barcode == pre_barcode:
            counts = count_read(first_almnt, second_almnt, genes, exons, gene_end, randomN_barcodes, barcode, counts)
        else:
            if len(counts) == 0:
                counts = count_read(first_almnt, second_almnt, genes, exons, gene_end, randomN_barcodes, barcode, counts)
                pre_barcode = barcode
...
def count_read(first_almnt, second_almnt, genes, exons, gene_end, randomN_barcodes, barcode, counts):
    ...
    if (not first_almnt.aligned) or (not second_almnt.aligned):
        ...
        if len(gene_id_intersect) == 1:
            ...
            gene_id = find_nearest_gene(first_almnt.iv.end_d, gene_id_intersect, gene_end, RT_barcode)
            counts[gene_id] += 1
        else:
            ...
            if len(gene_id_combine) == 1:
                ...
                gene_id = find_nearest_gene(first_almnt.iv.end_d, gene_id_combine, gene_end, RT_barcode)
                counts[gene_id] += 1
            else:
                ...
                if len(gene_id_intersect) == 0 and len(gene_id_combine) == 0:
                    gene_id_combine, gene_id_intersect = test_flanking_region(first_almnt, second_almnt, genes)
                if len(gene_id_intersect) == 0 and len(gene_id_combine) == 0:
                    gene_id_combine, gene_id_intersect = test_opposite_strand(first_almnt, second_almnt, exons, 1)
                    if len(gene_id_intersect) > 0 or len(gene_id_combine) > 0:
                        exonintron = ""
                if len(gene_id_intersect) == 0 and len(gene_id_combine) == 0:
                    gene_id_combine, gene_id_intersect = test_opposite_strand(first_almnt, second_almnt, genes, 2)
                ...
                if len(gene_id_intersect) == 1:
                    ...
                    gene_id = find_nearest_gene(first_almnt.iv.end_d, gene_id_intersect, gene_end, RT_barcode) + exonintron
                    counts[gene_id] += 1
                else:
                    # if there no intersection match, then find the union sets
                    if len(gene_id_combine) == 1:
                        ...
                        gene_id = find_nearest_gene(first_almnt.iv.end_d, gene_id_combine, gene_end, RT_barcode) + exonintron
                        counts[gene_id] += 1
                    else:
...
def find_nearest_gene(al_end, gene_id_intersect, gene_end, RT_barcode):
    ...
    # filter the gene with the least distance. If there are two genes with the least distance, then  "_ambiguous" would be returned
...
def test_flanking_region(first_almnt_tmp, second_almnt_tmp, genes_tmp):
    gene_id_intersect_tmp = set()
    gene_id_combine_tmp = set()
    inter_count_tmp = 0
    
    if second_almnt_tmp.iv.strand == "+":
        
        if (second_almnt_tmp.iv.start - 1000) >= 0:
            difference = 1000
        else:
            difference = second_almnt_tmp.iv.start
        ...
        if (first_almnt_tmp.iv.start - difference) < 0:
            difference = first_almnt_tmp.iv.start
